...
import { ChangePasswordData, changePasswordSchema } from "@/utils/schemas";
...
const ChangePasswordModal = ({ open, setOpen }: Props) => {
  ...
  const { control, handleSubmit, formState } = useForm<ChangePasswordData>({
        ...
        <View style={{ gap: 4 }}>
          ...
          <View>
            <Controller
              ...
              rules={{
                validate: (value, data) =>
                  value === data.new || "رمز عبور های جدید یکسان نیستند!",
              }}
              render={({ field: { value, onBlur, onChange } }) => (
                <Input
                  ref={newRepeatRef}
