...
import { ChangeEmailData, changeEmailSchema } from "@/utils/schemas";
...
const ChangeEmailModal = ({ open, setOpen }: Props) => {
  const toast = useToast();
  const { setUser, user } = useUserStore();
  const [loading, setLoading] = useState(false);

  const { control, handleSubmit, formState } = useForm<ChangeEmailData>({
